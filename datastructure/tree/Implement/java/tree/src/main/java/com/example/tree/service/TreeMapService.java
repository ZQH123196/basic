...
public class TreeMapService implements ITreeMapService {
     ...
     */
    @Override
    public MultipleTreeMap getTargetNodeChildren(String nodeName) {
        QueryWrapper<TreeNode> wrapper = Wrappers.<TreeNode>query()
    ...
    @Override
    public MultipleTreeMap getTreeByRoot(String rootName) {
    ...
    private static Map<String, TreeNodeVoMap> getChildNode(TreeNodeVoMap targetNode, Map<String, String> nodeMap) {
        ...
        nodeMap.forEach((nodeName, pathString) -> {
            if (pathString.substring(pathString.length() - 2, pathString.length() - 1).equals(targetNode.getNodeName())) {
                childNode.put(nodeName, TreeNodeVoMap.builder().nodeName(nodeName).nodePath(pathString).build());
            }
        });
        return childNode;
    }
